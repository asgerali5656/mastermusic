...
const LOCAL_STORAGE_HISTORY_KEY = "mastermusic_15day_play_history";
...
const getInitial15DayUniqueSong = (catalog: Song[]): Song => {
  if (!catalog || catalog.length === 0) return defaultGlobalSongs[0]!;
  const historyMap = getPersistentPlayHistory();
  const now = Date.now();
...
const getNextUniqueSong = (
  ...
  // 1. Filter candidate songs not played in 15 days and not currently queued
  ...
  // 2. Fallback: pick oldest played song from catalog not in current queue
  let oldestSong = catalog[0]!;
  let oldestTime = Infinity;
  for (const s of catalog) {
...
/** Synchronous Mobile Player Engine with 15-Day Unique Play Rule & Transparent Minimalist Search */
export function JukeboxPlayer({ stationSongs, stationId, apiSearchQuery, currentIndex, onTrackChange }: JukeboxPlayerProps) {
  ...
  useEffect(() => {
    ...
    searchYouTubeSongs(apiSearchQuery, 50).then(({ songs: liveSongs }) => {
  ...
  // Minimalist Transparent Search Panel State
  ...
  // Play a specific song directly from transparent search results
  ...
  // Record 15-day persistent history on track change
  useEffect(() => {
    if (currentTrack) {
          ...
          <div className="w-[min(94vw,36rem)] overflow-hidden rounded-2xl bg-black/40 p-3 backdrop-blur-md transition-all duration-300 animate-in fade-in slide-in-from-bottom-2">
            <form onSubmit={handleMinimalSearchSubmit} className="flex items-center gap-2 border-b border-white/20 pb-2">
              ...
              <input
                ...
                placeholder="Search any song or singer..."
        ...
        <div className="surface-glass flex w-[min(94vw,36rem)] items-center gap-3 rounded-full px-4 py-2.5 sm:px-5 sm:py-3 shadow-2xl backdrop-blur-2xl border border-white/15">
          ...
          <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
            ...
            <button
              ...
              title="Search Any Song (कोई भी गाना खोजीं)"
